...
Module MemoryFacts.
  ...
  Proof.
    ...
    - apply Interval.mem_ub. exploit VOLUME; try exact GET1; eauto. intros x. des; ss.
      inv x. congr.
    - econs; ss.
      + exploit VOLUME; try exact GET1; eauto. intros x. des; ss. inv x. congr.
      + left. ss.
  Qed.
  ...
  Lemma promise_exists_None
        ...
        (GET: Memory.get loc to promises1 = Some (from, Message.concrete val released))
        (LT: Time.lt from to):
  ...
  Lemma released_time_lt
    ...
    Time.lt from to.
  Proof.
    destruct (Cell.WF (mem loc)). exploit VOLUME; eauto. intros x. des; ss. inv x.
